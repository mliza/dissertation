...
def plot_stagnation_fields(data_in, fig_config, out_pdf_fig, cut_dict=None):
    ...
    for n in case_names:
        ...
        ## Plot Dielectric ## 
        plt.plot(noneq['x'] * 1E3, noneq_dielectric['dilute'] /
                 s_const.epsilon_0, color=colors[0],
            linewidth=fig_config['line_width'], label='Nonequilibrium')

        plt.plot(frozen['x'] * 1E3, frozen_dielectric['dilute'] /
                 s_const.epsilon_0,
                 color=colors[1],
                 linewidth=fig_config['line_width'], label='Frozen')

        plt.xlabel('X $[mm]$', fontsize=fig_config['axis_label_size'])
        plt.ylabel('$ \\epsilon_m / \\epsilon_0$ $[ ]$', fontsize=fig_config['axis_label_size'])
        ...
        if cut_dict and 'temperature' in cut_dict[n]:
            plt.xlim(cut_dict[n]['temperature'])
        plt.savefig(os.path.join(out_pdf_fig, f'{n}_dielectricMedium.pdf'),
                    format='pdf', bbox_inches='tight',
                    dpi=fig_config['dpi_size'])
        plt.close() 
        ## Plot Dielectric ## 
